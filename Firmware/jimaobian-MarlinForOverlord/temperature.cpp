...
unsigned char fanSpeedSoftPwmOveride;
...
void manage_heater()
{
  ...
  static uint8_t powerOnDemandStateBackUp;

  if (powerOnDemandState<PowerOnDemandOutOfPower && votageDetection<(625*OVERSAMPLENR)) {
    powerOnDemandStateBackUp=powerOnDemandState;
    powerOnDemandState=PowerOnDemandOutOfPower;
  ...
  case PowerOnDemandOutOfPower:
    if (millis()-powerOnDemandTimer>2000) {
      ...
      currentMenu=lcd_menu_power_check;
      powerOnDemandState=PowerOnDemandOutOfPowerStopping;
      powerOnDemandTimer=millis();
    }
    else if (votageDetection>=(625*OVERSAMPLENR))
    {
      powerOnDemandState=powerOnDemandStateBackUp;
    }
    break;
  case PowerOnDemandOutOfPowerStopping:
    if ((movesplanned()==0 && is_command_queued()==0 && isCommandInBuffer()==0) || (millis()-powerOnDemandTimer>10000)) {
      quickStop();
      discardEnqueueingCommand();
      discardCommandInBuffer();
      disable_x();
      ...
      st_sleep();
      newPowerSleep();
      powerOnDemandState=PowerOnDemandOutOfPowerSleeping;
    }

    break;
  case PowerOnDemandOutOfPowerSleeping:
    if (votageDetection<(625*OVERSAMPLENR)) {
      powerOnDemandTimer=millis();
    }
    else{
      if (millis()-powerOnDemandTimer>2000) {
  ...
  for(int e = 0; e < EXTRUDERS; e++)
  {
    ...
    if(pid_error[e] > 60) {
      ...
      pid_output = PID_MAX-PID_CLOSE_RESTRICT;
      pid_reset[e] = true;
    }
...
void tp_init()
{
  ...
  #endif
  #ifdef FAN_SOFT_PWM
  if (fanSpeedSoftPwmOveride) {
    soft_pwm_fan=fanSpeedSoftPwmOveride / 2;
  }
  else{
...
#endif
#if defined(HEATER_BED_PIN) && HEATER_BED_PIN > -1
    soft_pwm_b = soft_pwm_0+soft_pwm_bed;
...
#if defined(TEMP_1_PIN) && (TEMP_1_PIN > -1) && ((EXTRUDERS > 1) || defined(TEMP_SENSOR_1_AS_REDUNDANT))
  #if TEMP_1_PIN > 7
    ADCSRB = 1<<MUX5;
  #else
    ADCSRB = 0;
