...
public:
  ...
  FORCE_INLINE bool eof() {
    return file.curPosition()>=filesize;
  };
  FORCE_INLINE int16_t get() {
    sdpos = file.curPosition(); return (int16_t)file.read();
  };
  FORCE_INLINE int16_t fgets(char* str, int16_t num) {
    sdpos = file.curPosition(); return file.fgets(str, num);
  }
  FORCE_INLINE void setIndex(long index) {
    sdpos = index; file.seekSet(index);
  };
  FORCE_INLINE uint8_t percentDone(){
    if(!isFileOpen()) return 0; if(filesize) return sdpos/((filesize+99)/100); else return 0;
  };
  FORCE_INLINE char* getWorkDirName(){
    workDir.getFilename(filename); return filename;
  };
  FORCE_INLINE bool atRoot() {
    return workDirDepth==0;
  }
  FORCE_INLINE uint32_t getFilePos() {
    return sdpos;
  }
  FORCE_INLINE uint32_t getFileSize() {
    return filesize;
  }
  FORCE_INLINE bool isOk() {
    return cardOK && card.errorCode() == 0;
  }
  FORCE_INLINE int errorCode() {
    return card.errorCode();
  }
  FORCE_INLINE void clearError() {
    card.error(0);
  }
